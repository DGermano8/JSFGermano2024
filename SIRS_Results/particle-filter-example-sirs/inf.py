...
from plotnine import ggplot, geom_rect, aes, geom_ribbon, geom_point, scale_y_continuous, scale_x_continuous, labs, theme_bw, geom_vline
import pdb
...
def plottable_model_cis(model_ci_df : pd.DataFrame) -> pd.DataFrame:
    ...
    # tmp['xmax'] = tmp['xmax'] / tmp['xmax'].max()
...
def state_plt_p9(post_df: pd.DataFrame,
    ...
    return (ggplot()
            ...
            + scale_y_continuous(name = "Infectious population size")
...
def sirs_run_inference(inf_ctx : pypfilt.Context) -> Dict[str, Any]:
    """
    ...
    # param_names = ["omegaCoef", "kappaCoef", "muCoef"]
