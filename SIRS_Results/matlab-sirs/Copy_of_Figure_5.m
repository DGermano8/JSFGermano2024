...
for ii=1:HowManySimsToDo
    ...
    DATA = readmatrix([master_path,'/JSF_N0Index_', num2str(N0ii) , '_iiSeed_', num2str(ii) ,'.csv']);
    % DATA = readmatrix([master_path,'/JSF_T3_N0Index_', num2str(N0ii) , '_iiSeed_', num2str(ii) ,'.csv']);
    if(max(DATA(:,3)) < 50 )
        JSF_Ext = JSF_Ext + 1;
    ...
    DATA = readmatrix([master_path,'/GIL_N0Index_', num2str(N0ii) , '_iiSeed_', num2str(ii) ,'.csv']);
    if(max(DATA(:,3)) < 50 )
        Gil_Ext = Gil_Ext + 1;
...
f1=figure; 
hold on;
% plot(TimeObserve, ones(length(TimeObserve),1),'--', 'color', 0.5*[1 1 1], 'LineWidth', 1.75);
...
ylabel('Infections $$(\times 10^{3})$$','FontSize',18, 'Interpreter','Latex')
...
ax.FontSize = 14;
xlabel('-','FontSize',18, 'Interpreter','Latex','color','white')
ylabel('Peak infection time (days)','FontSize',18 , 'Interpreter','Latex')
...
ax.XAxis.Color = 'white';
ax.FontSize = 14;
ylabel('Peak infective $$(\times 10^{3})$$','FontSize',18, 'Interpreter','Latex')
xlabel('-','FontSize',18, 'Interpreter','Latex','color','white')
box off;
...
ax.FontSize = 14;
xlabel('Time (days)','FontSize',18, 'Interpreter','Latex')
ylabel('Cumulative Infections $$(\times 10^{5})$$','FontSize',18, 'Interpreter','Latex')
f1.Position = [326   201   311   273];

f3=figure;
v = violinplot(FinalCum_JSF/(10^5),ones(size(FinalCum_JSF)), 'QuartileStyle','boxplot', 'HalfViolin','right','Showdata',false);
v(1,1).ViolinPlot.FaceColor = JSF_colour;
v(1,1).ViolinPlot.EdgeColor = JSF_colour;
v(1,1).ViolinPlot.FaceAlpha = 0.6;
axis([1 1.5 0 18])
...
f3=figure;
v = violinplot(FinalCum_Gil/(10^5),ones(size(FinalCum_Gil)), 'QuartileStyle','boxplot', 'HalfViolin','right','Showdata',false);
v(1,1).ViolinPlot.FaceColor = Gil_colour;
v(1,1).ViolinPlot.EdgeColor = Gil_colour;
v(1,1).ViolinPlot.FaceAlpha = 0.6;
axis([1 1.5 0 18])
